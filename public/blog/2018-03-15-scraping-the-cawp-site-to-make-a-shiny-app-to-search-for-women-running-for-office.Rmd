---
title: Scraping The CAWP Site To Make a Shiny App To Search For Women Running For
  Office
author: "Jenny Listman"
date: '2018-03-15'
slug: scraping-the-cawp-site-to-make-a-shiny-app-to-search-for-women-running-for-office
tags:
- shiny
- webscraping
- statebins
...
---
####By [Jenny Listman](/)
#####An Interactive App To Visualize Women Runnning for State or Federal Offices
![](img/CAWPmap.jpg)
...
See the app in action [here](https://jennylistman.shinyapps.io/WomenCandidates/). Read a blog about it [here](https://medium.com/@jblistman/an-app-to-search-for-women-running-for-office-in-2018-346f5a013ec9). The design of the app was inspired by the [adavis](https://gokhan.shinyapps.io/adavis/) app from Gokhan Ciflikli 

Load packages needed:

```{r load_packages, eval = FALSE, results = 'hide', message=FALSE}
library(rvest)
library(tidyverse)
library(janitor)
library(shiny)
...
```{r scrape_data, results = "hide", eval = FALSE}
...
```{r make_data, results = "hide", message=FALSE, eval = FALSE}
...
```{r fix_variable, results = "hide", message=FALSE, warning= FALSE, eval = FALSE}
...
```{r clean_strings, results = "hide", eval = FALSE}
elections <- elections %>%
...
```{r make_variables, results = "hide", eval = FALSE}
...
`rvest::html_table` takes the text from a website's table, but doesn't get hyperlinks to elements in the table. I scraped links to candidate websites to add them to the candidate name variable in the dataframe so there could be live hyperlinks in the `shiny` app datatable, itself.
![](img/CAWPdatatable.jpg)
...
```{r make_mapdata, results = "hide", eval = FALSE}
...
```{r complete_mapdata, results = "hide", eval = FALSE}
...
```{r map_colors, results = "hide", eval = FALSE}
