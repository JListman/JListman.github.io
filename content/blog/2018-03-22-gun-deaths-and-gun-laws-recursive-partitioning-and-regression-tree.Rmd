...
#####This project uses a Recursive Partitioning And Regression Tree analysis from the `rpart` package to examine the effect of 49 gun law subcategories on annual State firearm death rates. Ten subcategories were found to significantly predict State gun death rate decreases or increases. Data files needed to run this code can be found in my [GitHub repo](https://github.com/JListman/FirearmDeathRate) associated with the project.Read the blog post about it [here](https://medium.com/@jblistman). 
=======
####This project uses a Recursive Partitioning And Regression Tree analysis from the `rpart` package to examine the effect of 49 gun law subcategories on annual State firearm death rates. Ten subcategories were found to significantly predict State gun death rate decreases or increases. Data files needed to run this code can be found in my [GitHub repo](https://github.com/JListman/FirearmDeathRate) associated with the project.Read the blog post about it [here](https://medium.com/@jblistman). 
>>>>>>> parent of 283874a... change blog intros

![](img/GunDeathTreeForWebsite.jpg)
...
2. Download CDC cause of death by State from firearms 1999 to 2016 data. On the [CDC Wonder](https://wonder.cdc.gov/ucd-icd10.html) website, sort by State, Year, and cause of injury = Firearm. This produces a file that contains the crude death rate per 100,000 state residents due to firearm per state per year. This was saved as StateGunDeathRate.csv
...
```{r, results = 'hide', message=FALSE, fig.show="hide", eval = FALSE}
