...
```
####By [Jenny Listman](/)
#####This post describes R code to process, analyse, and visualize data from NYC's 311 database.
...
RatMouseData <- RatMouseData %>% 
        clean_names() %>% 
        mutate_at(cols, funs(factor(.))) 
```

Make a new dataframe that includes only the subset of rows for which Descriptor variable is "Rat Sighting". 
...
For each Month_Year create a map. This code was adapted from a [tutorial by Kimberly Coffey](http://www.kimberlycoffey.com/blog/2015/10/17/animated-choropleth-map-for-the-web). First, make an empty list that will then be filled with maps. The loop iterates over each Month_Year to create a map. The entire `TidyRatMapData` must be subsetted by Month_Year (`temp.date`) for each map. Include a variable `totalrats` that sums all rat complaints for a given Month_Year so this value can be included in the title of each image.
In `scale_fill_manual` and `guides`, override the vector of color codes that correspond to binned values so that zip codes with no rat complaints for a given month are colored gray.
